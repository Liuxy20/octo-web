...
- 灰色总数和完整群归属依赖 octo-server#904 的可选 sideband；旧服务端不返回字段时，灰色数字只能反映本次 Web 会话内观察到的累计值，其他端已读后可能不会下降。
...
- Base 全量测试：543 个文件、5634 个用例通过；其中定向测试为 5 个文件、34 个用例。覆盖实时更新、去重、账号切换重置、同步与实时消息竞态、Space 前缀归属、静音元数据缺失、关闭弹窗清除和 `99+`。
